...
Depending on your language of choice, there are many ways of doing this. In the case of C++, one way is to initialize a stringstream with this input string, and keep pushing words out of the stringstream one by one until it's empty. This words because stringstreams delimit by spaces, meaning the end of a input can be denoted by a space in a stringstream. The code for this would be like so (in C++):
